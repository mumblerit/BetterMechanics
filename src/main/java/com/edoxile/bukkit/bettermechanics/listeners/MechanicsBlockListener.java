...
public class MechanicsBlockListener extends BlockListener {
	...
	public void onSignChange(SignChangeEvent event) {
		...
		if (str.equalsIgnoreCase("[lift up]")) {
			...
			event.getPlayer().sendMessage(
					ChatColor.AQUA + "You created a small gate!");
		} else if (str.equalsIgnoreCase("[bridge]")) {
			event.setLine(1, "[Bridge]");
			event.getPlayer().sendMessage(
					ChatColor.AQUA + "You created a bridge!");
			...
			event.getPlayer().sendMessage(
					ChatColor.AQUA + "You created a door!");
			...
			event.getPlayer().sendMessage(
					ChatColor.AQUA + "You created a small door!");
			...
			event.setLine(1, "[X]");
			event.getPlayer().sendMessage(
					ChatColor.AQUA + "You created a hidden switch!");
		} else {
			return;
		}
	}
