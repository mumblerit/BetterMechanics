...
public class BlockMapper {
	...
	public static HashSet<Block> mapHorizontal(BlockFace direction,
			Block start, Block end, boolean small)
		...
		case WEST: {
			...
			if (startLoc.getBlockX() != endLoc.getBlockX()
					|| startLoc.getBlockY() != endLoc.getBlockY()
					|| startLoc.getBlockZ() > endLoc.getBlockZ()) {
		...
		case EAST: {
			...
			if (startLoc.getBlockX() != endLoc.getBlockX()
					|| startLoc.getBlockY() != endLoc.getBlockY()
					|| endLoc.getBlockZ() > startLoc.getBlockZ()) {
				throw new InvalidDirectionException();
			} else {
				Block tempBlock = start;
				while (tempBlock != end) {
		...
		case SOUTH: {
			...
			if (startLoc.getBlockZ() != endLoc.getBlockZ()
					|| startLoc.getBlockY() != endLoc.getBlockY()
					|| startLoc.getBlockX() > endLoc.getBlockX()) {
				throw new InvalidDirectionException();
			} else {
				Block tempBlock = start;
				while (tempBlock != end) {
		...
		case NORTH: {
			...
			if (startLoc.getBlockZ() != endLoc.getBlockZ()
					|| startLoc.getBlockY() != endLoc.getBlockY()
					|| endLoc.getBlockX() > startLoc.getBlockX()) {
				throw new InvalidDirectionException();
			} else {
				Block tempBlock = start;
				while (tempBlock != end) {
	...
	public static HashSet<Block> mapVertical(BlockFace direction,
			BlockFace orientation, Block start, Block end, boolean small)
			throws InvalidDirectionException {
		HashSet<Block> blockSet = new HashSet<Block>();
		switch (direction) {
		case UP: {
	...
	public static HashSet<Block> mapAllInCuboidRegion(Block start, int sw,
			Material m) {
		Block tempBlock;
		HashSet<Block> blockSet = new HashSet<Block>();
		int nsw = ~sw + 1;
	...
	public static HashSet<Block> mapFlatRegion(Block start, Material m, int w,
			int l) throws OutOfBoundsException {
		Block tempBlock = start;
	...
	public static Sign findMechanicsSign(Block block, BlockFace direction,
			MechanicsType type, int maxBlockDistance)
