...
public class Cauldron {
	public static Cauldron preCauldron(Block clickedBlock, MechanicsConfig c,
			Player p) {
		...
		for (dy = -1; dy >= -3; dy--) {
			...
			if (tempBlock.getType() == Material.LAVA
					|| tempBlock.getType() == Material.STATIONARY_LAVA)
				break;
		}
		if (dy > -4) {
			// Search for cauldron walls
			if (clickedBlock.getRelative(BlockFace.WEST).getType() == Material.STONE
					&& clickedBlock.getRelative(BlockFace.EAST)
							.getRelative(BlockFace.EAST).getType() == Material.STONE) {
				dz = -1;
			} else if (clickedBlock.getRelative(BlockFace.WEST)
					.getRelative(BlockFace.WEST).getType() == Material.STONE
					&& clickedBlock.getRelative(BlockFace.EAST).getType() == Material.STONE) {
				dz = 0;
			} else {
				return null;
			}
			if (clickedBlock.getRelative(BlockFace.NORTH).getType() == Material.STONE
					&& clickedBlock.getRelative(BlockFace.SOUTH)
							.getRelative(BlockFace.SOUTH).getType() == Material.STONE) {
				dx = 0;
			} else if (clickedBlock.getRelative(BlockFace.NORTH)
					.getRelative(BlockFace.NORTH).getType() == Material.STONE
					&& clickedBlock.getRelative(BlockFace.SOUTH).getType() == Material.STONE) {
				dx = -1;
			} else {
				return null;
			}
				...
				for (int ndx = 0; ndx <= 1; ndx++) {
					for (int ndz = 0; ndz <= 1; ndz++) {
						if (referencePoint.getRelative(ndx, ndy, ndz).getType() != Material.AIR) {
							blockSet.add(referencePoint.getRelative(ndx, ndy,
									ndz));
						}
					}
				}
	...
	public boolean performCauldron() {
		...
		if (recipe != null) {
			...
			do {
				iterator.next();
				HashMap<Integer, ItemStack> inventoryMap = player
						.getInventory()
						.addItem(
								new ItemStack(iterator.key(), iterator.value()));
				for (Map.Entry<Integer, ItemStack> i : inventoryMap.entrySet()) {
					player.getLocation().getWorld()
							.dropItem(player.getLocation(), i.getValue());
				}
