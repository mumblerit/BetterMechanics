...
public class TeleLift {
	...
	public boolean map() throws NumberFormatException {
		...
		if (destination.getY() <= 127) {
			...
			if (canPassThrough(dest1.getType())
					&& canPassThrough(dest2.getType())) {
	...
	public boolean movePlayer() {
		if (player.teleport(destination)) {
			player.sendMessage(ChatColor.GOLD
					+ "You magically teleported to an other location!");
			return true;
		} else {
			player.sendMessage(ChatColor.RED
	...
	public Location parseDestination() throws NumberFormatException {
		...
		String[] locations = (sign.getLine(2).concat(sign.getLine(3)))
				.split(":");
		if (locations.length == 3) {
			...
			if (newLocation.getY() < 2) {
				player.sendMessage(ChatColor.RED
						+ "Invalid Y location. Teleporting to y = 130");
